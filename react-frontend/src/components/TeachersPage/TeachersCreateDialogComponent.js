...
const TeachersCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            teacherId: _entity?.teacherId,name: _entity?.name,department: _entity?.department,yearsOfExperience: _entity?.yearsOfExperience,
        ...
        const result = await client.service("teachers").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Teachers created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Teachers" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Teachers" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="teachers-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputNumber id="yearsOfExperience" className="w-full mb-3 p-inputtext-sm" value={_entity?.yearsOfExperience} onChange={(e) => setValByKey("yearsOfExperience", e.value)}  />
            </span>
            <small className="p-error">
